...
class CTGovSearcher:
    ...
    def _clamp_page_size(page_size: int) -> int:
        ...
        value = 1 if page_size is None else int(page_size)
        return max(1, min(value, MAX_PAGE_SIZE))

    @staticmethod
    def _normalize_nct_ids(nct_ids: Iterable[str]) -> Tuple[List[str], List[str]]:
    ...
    def search(self, condition: str, strategy: str = "S1",
               intervention: str = None, page_size: int = 100,
               return_studies: bool = False) -> SearchResult:
        """
        ...
        if strategy not in self.STRATEGIES:
            raise ValueError(f"Unknown strategy: {strategy}. Valid: {list(self.STRATEGIES.keys())}")

        strat = self.STRATEGIES[strategy]
        query = strat["build_query"](condition, intervention)
        params = build_params(query)
        params["countTotal"] = "true"
        effective_page_size = self._clamp_page_size(page_size if return_studies else 1)
        params["pageSize"] = effective_page_size

        query_url = CTGOV_API
        start_time = time.time()
        try:
            response = self.session.get(CTGOV_API, params=params, timeout=self.timeout)
    ...
    def compare_all_strategies(self, condition: str,
                                intervention: str = None) -> List[SearchResult]:
        """
        ...
        for strategy_id in self.STRATEGIES.keys():
            result = self.search(condition, strategy=strategy_id, intervention=intervention)
            results.append(result)
            time.sleep(RATE_LIMIT_DELAY)  # Rate limiting
        return results

    def search_with_synonyms(self, condition: str, strategy: str = "S1") -> SearchResult:
        """
        ...
        strat = self.STRATEGIES[strategy]
        synonyms = self.synonyms.get(condition.lower(), [])
        all_terms = [condition] + synonyms

        # Build OR query
        or_query = " OR ".join([f'"{term}"' for term in all_terms])

        params = build_params(strat["build_query"](condition, None))
        if strategy == "S9":
    ...
    def validate_nct_ids(self, nct_ids: List[str]) -> Dict[str, bool]:
        ...
        results = {nct_id: False for nct_id in invalid_ids}

        if not valid_ids:
            return results

        try:
            found_ids = fetch_matching_nct_ids(
    ...
    def calculate_recall(self, condition: str, known_nct_ids: List[str],
                         strategy: str = "S1") -> RecallMetrics:
        """
        ...
        query = self.STRATEGIES[strategy]["build_query"](condition, None)       
        params = build_params(query)
        found_nct_ids = fetch_matching_nct_ids(
            self.session, params, known_nct_ids, timeout=self.timeout
        )

        known_set = {nct.upper().strip() for nct in known_nct_ids if nct}
        found = known_set.intersection(found_nct_ids)
        missed = known_set - found_nct_ids

        recall = len(found) / len(known_set) * 100 if known_set else 0
    ...
    def search_by_nct_ids(self, nct_ids: List[str]) -> SearchResult:
        ...
        try:
            ...
            for start in range(0, len(valid_ids), batch_size):
                ...
                params = {"query.id": " OR ".join(batch)}
                batch_page_size = self._clamp_page_size(len(batch))
    ...
    def export_results_csv(self, results: List[SearchResult], filepath: str):
        """Export search results to CSV file."""
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['strategy_id', 'strategy_name', 'condition',
                           'total_count', 'execution_time', 'query_url', 'error'])
            for r in results:
                writer.writerow([r.strategy_id, r.strategy_name, r.condition,
                               r.total_count, r.execution_time, r.query_url, r.error or ''])

    def generate_search_report(self, condition: str) -> str:
        """Generate a formatted report for a condition search."""
        results = self.compare_all_strategies(condition)
        baseline = results[0].total_count if results else 0
        ...
        report.append(f"{'Strategy':<5} {'Name':<35} {'Count':>10} {'% Baseline':>12}")
        report.append("-" * 65)

        for r in results:
            pct = f"{(r.total_count / baseline * 100):.1f}%" if baseline > 0 else "N/A"
            report.append(f"{r.strategy_id:<5} {r.strategy_name:<35} {r.total_count:>10,} {pct:>12}")
...
def main():
    ...
    syn_result = searcher.search_with_synonyms("diabetes")
    print(f"   Total with synonyms: {syn_result.total_count:,}")

    # Validate NCT IDs
    print("\n4. NCT ID Validation (sample):")
    sample_ncts = ["NCT03702452", "NCT00400712", "NCT99999999"]
    validation = searcher.validate_nct_ids(sample_ncts)
